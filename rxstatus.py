...
def main(logfilepath, csvwritepath):
    ...
    with open(logfilepath, 'r') as f:
        while True: 
            ...
            if line.startswith("<RXSTATUS COM1") or line.startswith("[COM1]<RXSTATUS COM1"):
                dataline = f.readline()
                dataline = tuple(dataline[1:].strip().split(" "))
                #print(dataline)
...
if __name__ == '__main__':
    ...
    for serial in serials:
        for session in range(1,3):
            ...
            main(logfilepath, os.path.join(csvwritepath, "RXSTATUS.csv"))
